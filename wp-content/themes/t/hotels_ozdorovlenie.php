<?php
/*
Template Name: hotels ozdorovlenie
*/ ?>
...
<script src="http://37.18.77.200/search_tour?samo_action=embed&LANG=rus&DOLOAD=1&TOURTYPE=6&NIGHTS_FROM=4&NIGHTS_TILL=7" charset="windows-1251"></script>
<style>
.CHILD{
position: absolute;
    top: 146px;
...
$(tr1_2[3]).css('left','180px');
		...
		if (bus%2==1)
		{
			...
			for (var tour_type=0; tour_type<tour.length; tour_type++)
			{
				...
				for (var tour_word=0; tour_word<tour_travel.length; tour_word++)
				{
					...
					if (tour_travel[tour_word]=='(автобус)')
					{
						bus_count++;
					}
				}
	...
	$('.plane').click(function(){
		...
		if (plane%2==1)
		{
			...
			for (var tour_type=0; tour_type<tour.length; tour_type++)
			{
				...
				{
					//console.log(tour_travel[tour_word]);
					if (tour_travel[tour_word]=='(авиа)' || tour_travel[tour_word]=='(самолет)')
					{
						plane_count++;
					}
				}
	...
	function second_passed(){
				...
				for (var count=0; count<=9; count++)
				{
					...
					$(nights_block[0]).html(tour_name2);
					...
					var tour_time=tour_durible.split('');
					console.log(tour_time);
					var new_tour_time=tour_durible;
					$(nights_block[9]).html(new_tour_time+'<br>');
					...
					$(ul[2]).html('<img src="http://hotels.bolgaria-travel.com/wp-content/uploads/2016/12/check.png">Проживание '+update_tour_durible+' ночей');
